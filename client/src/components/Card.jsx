...
const Card = ({ _id, name, prompt, photo }) => {
  return (
    <div className="group rounded-xl relative shadow-card hover:shadow-cardhover card">
      ...
      >
        <p className="text-white text-md overflow-y-auto prompt font-semibold">
          {prompt}
        </p>
        <div className="mt-5 flex justify-between items-center gap-2">
          <div className="flex items-center gap-2">
            <div
              className="w-7 h-7 rounded-full object-cover bg-pink-500 flex 
            justify-center items-center text-white text-xs font-bold"
            >
              {name[0]}
            </div>
            <p className="text-white text-sm font-bold italic">{name}</p>
          </div>
          <button
